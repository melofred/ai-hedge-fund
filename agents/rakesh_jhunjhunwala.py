...
def rakesh_jhunjhunwala_agent(state: AgentState, agent_id: str = "rakesh_jhunjhunwala_agent"):
    ...
    for ticker in tickers:
        ...
        if elapsed_time > max_execution_time:
            ...
            print(f"   📊 Processed tickers: {len(jhunjhunwala_analysis)}")
            print(f"   ⚠️  Skipping remaining tickers - JHUNJHUNWALA ANALYSIS INCOMPLETE!")
            print(f"   📊 Missing Jhunjhunwala analysis for: {', '.join(tickers[len(jhunjhunwala_analysis):])}")
    ...
    processed_tickers = len(jhunjhunwala_analysis)
